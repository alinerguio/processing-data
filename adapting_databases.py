...
def explore_dir(directory):
    sub_list_dir = os.listdir(directory)
    for element in sub_list_dir:
        if '.wav' in element or '.txt' in element:
            yield directory + '/' + element
        elif '.' not in element and element not in ['LICENSE', 'README', 'PROMPTS','prompts-original', 'audiofile_details']:
            yield from explore_dir(directory + '/' + element)
...
if __name__ == '__main__':
    ...
    databases = ['adapted_laps_dataset']
